...
# Loading, splitting, preprocessing data -- RAW version (no scaling)
...
class MoleculeDataset(Dataset):
    def __init__(self, target_path: str, noisy_path: str, indices_path: str = None, scaler=None):
        ...
        # ---- Merge exactly like original loader ----
    ...
    def __getitem__(self, idx):
        x = torch.cat([self.fp[idx], self.noisy[idx]], dim=0)  # (2049,)
        return x, self.y[idx]
...
def get_dataloaders(
    ...
    dataset = MoleculeDataset(dataset_path, noisy_path, indices_path=indices_dir, scaler=None)
